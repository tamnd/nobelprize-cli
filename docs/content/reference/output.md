...
describes what users get. Pick a format with `-o`, or let nobelprize choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nobelprize <command> -o table   # aligned columns for reading
nobelprize <command> -o jsonl   # one JSON object per line, for piping
nobelprize <command> -o json    # a single JSON array
nobelprize <command> -o csv     # spreadsheet friendly
nobelprize <command> -o tsv     # tab-separated
nobelprize <command> -o url     # just the URL column
nobelprize <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nobelprize <command> --fields id,title,url
```
...
```bash
nobelprize <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nobelprize <command>            # a table, because this is a terminal
nobelprize <command> | wc -l    # JSONL, because this is a pipe
```
